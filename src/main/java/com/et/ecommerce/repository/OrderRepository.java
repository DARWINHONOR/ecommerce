...
import com.et.ecommerce.dto.OrderItemDto;
...
public interface OrderRepository extends JpaRepository<Order, UUID> {
  ...
  @Query("SELECT new com.et.ecommerce.dto.OrderItemDto(p.id, oi.quantity, (p.price * oi.quantity)) "
      + "FROM Order o "
      + "JOIN o.items oi "
      + "JOIN oi.product p "
      + "WHERE o.id = ?1")
  List<OrderItemDto> getItemWithTotalPrice(UUID orderId);
}
